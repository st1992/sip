...
func TestBuildSiprecSDP(t *testing.T) {
	...
	// Session ID should be modified (no longer exactly 151827427)
	assert.NotContains(t, sdp2, "151827427 151827427")
}
...
func TestSiprecSessionStore_GetByLegCallID(t *testing.T) {
	...
	store.Set("test-call-id", session)
	store.RegisterLeg("leg-a-call-id", "test-call-id")
	store.RegisterLeg("leg-b-call-id", "test-call-id")
