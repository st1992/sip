...
// Uses a random suffix to avoid collision with the original or other legs.
...
func modifySiprecOriginLine(originLine string) string {
	...
	if _, err := rand.Read(b); err != nil {
		...
		if sessID, err := strconv.ParseInt(parts[1], 10, 64); err == nil {
			parts[1] = fmt.Sprintf("%d%s", sessID, suffix)
		} else {
...
// It also ensures each media block has its own c= line (required when the two
// legs may have different connection addresses).
func writeSiprecMediaBlockWithRecvOnly(b *strings.Builder, media []SDPLine, label string) {
	...
	// Write the m= line first, then inject c= if missing (right after m= per SDP ordering)
	mLineWritten := false
	for _, line := range media {
		if line.Type == 'm' {
			b.WriteString(line.Value + "\r\n")
			mLineWritten = true
			if !hasConnection {
				// Will be filled by session-level c= via BuildCombinedSiprecSDP;
				// no injection needed since the session c= applies.
			}
			continue
		}
		if line.Type == 'a' {
		...
		_ = mLineWritten // suppress unused warning
		b.WriteString(line.Value + "\r\n")
	}
