...
const (
	...
	// siprecCleanupInterval is how often the stale session cleanup runs
	siprecCleanupInterval = 1 * time.Minute
)

// StartSiprecCleanup launches a background goroutine that periodically removes
// stale SIPREC sessions. It stops when ctx is cancelled.
func (s *Server) StartSiprecCleanup(ctx context.Context) {
	go func() {
		ticker := time.NewTicker(siprecCleanupInterval)
		defer ticker.Stop()
		for {
			select {
			case <-ctx.Done():
				return
			case <-ticker.C:
				cleaned := s.siprecSessions.CleanupStale(siprecSessionTTL, func(session *SiprecSession) {
					s.log.Warnw("Cleaning up stale SIPREC session", nil, "sipCallID", session.ID)
				...
				if cleaned > 0 {
					s.log.Infow("SIPREC stale session cleanup complete", "cleaned", cleaned)
				}
			}
		}
	}()
...
func (s *Server) handleSiprecCall(
	...
	s.siprecSessions.RegisterLeg(callIDA, session.ID)
	s.siprecSessions.RegisterLeg(callIDB, session.ID)
	...
	// Wait for both legs (WaitForResults blocks until both complete or ctx expires)
...
// It reuses the session-level auth and dispatch results (stored in session.SharedDispatch)
// to avoid redundant auth/dispatch RPCs per leg.
func (s *Server) processSiprecLeg(
	...
	if session.SharedDispatch == nil {
		return "", errors.New("session dispatch not available")
	}
	disp := *session.SharedDispatch
	...
	if disp.ProjectID != "" {
		log = log.WithValues("projectID", disp.ProjectID)
	}
	if disp.TrunkID != "" {
		log = log.WithValues("sipTrunk", disp.TrunkID)
	}
	if disp.DispatchRuleID != "" {
		log = log.WithValues("sipRule", disp.DispatchRuleID)
	}

	state := NewCallState(s.getIOClient(disp.ProjectID), &livekit.SIPCallInfo{
		CallId:        callID,
		Region:        s.region,
	...
	state.Update(ctx, func(info *livekit.SIPCallInfo) {
		...
		info.RoomName = session.SharedRoomName
	...
	legLog := LoggerWithHeaders(log, legCC)
	legCC.log = legLog
	...
	siprecAttrs := map[string]string{
		...
		livekit.AttrSIPTrunkID:     disp.TrunkID,
	...
	// Build room config from shared dispatch
	roomConf := disp.Room
	roomConf.RoomName = session.SharedRoomName

	// Each leg gets its own unique participant identity
	...
	// Fresh attributes map (don't share reference between legs)
...
func (s *Server) runSiprecLegMedia(
	...
	mconf.Processor = s.handler.GetMediaProcessor(disp.EnabledFeatures, disp.FeatureFlags, string(call.cc.ID()))
