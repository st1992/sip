...
// Each answer's session-level c= line is injected into its media block so
// the combined SDP correctly addresses each leg's media endpoint.
func CombineSiprecAnswerSDPs(originalOfferSDP string, answerSDPA, answerSDPB string) (string, error) {
	...
	// Ensure each media block carries a media-level c= line from its answer.
	// This is critical when the two legs bind to different IPs.
	mediaA := ensureMediaConnection(mediaBlocksA[0], sessionA)
	mediaB := ensureMediaConnection(mediaBlocksB[0], sessionB)

	combinedSDP := BuildCombinedSiprecSDP(sessionA, mediaA, mediaB, labelA, labelB)
	return combinedSDP, nil
}

// ensureMediaConnection checks whether the media block already has a c= line.
// If not, it copies the session-level c= line into the media block (inserted
// right after the m= line per SDP ordering conventions).
func ensureMediaConnection(media []SDPLine, session []SDPLine) []SDPLine {
	for _, line := range media {
		if line.Type == 'c' {
			return media
		}
	}
	// Find session-level c= line
	...
	// Insert c= right after the m= line
